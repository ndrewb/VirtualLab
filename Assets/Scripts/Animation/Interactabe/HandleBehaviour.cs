...
public class HandleBehaviour : MonoBehaviour
{
    ...
    private void OnMouseDrag()
    {
        ...
        if (Physics.Raycast(ray, out hit))
        {
            ...
            objPosition.z =
                0; // ”становите нужное фиксированное значение Z, если объект должен перемещатьс€ в плоскости, отличной от 0
        ...
        if (!SceneBehaviour.ForcedVolumeHold && !SceneBehaviour.VolumeFixed)
            transform.position =
                Vector3.Lerp(transform.position, targetPosition,
                    Time.deltaTime * 5); // 5 - скорость перемещени€, можно настроить
    }
